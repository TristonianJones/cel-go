...
func testCost(t *testing.T, expr string, estimatedCost cost.CostEstimate, runtimeCost uint64, estimatedCostV0 ...cost.CostEstimate) {
	...
	legacyEst, err := env.EstimateCost(checkedAst, &noopCostEstimator{}, cost.EstimateModelVersion(cost.ModelVersion0))
	if err != nil {
		t.Fatalf("env.EstimateCost(%q, ModelVersion0) failed: %v", expr, err)
	}
	if legacyEst.Min != wantLegacy.Min || legacyEst.Max != wantLegacy.Max {
		t.Errorf("expected ModelVersion0 estimated cost %v, got %v for expr %q", wantLegacy, legacyEst, expr)
	}
