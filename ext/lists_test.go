...
func TestLists(t *testing.T) {
	listsTests := []struct {
		...
		{expr: `[1, 2, 3].hasAny([2, 4]) == true`},
		{expr: `[1, 2, 3].hasAny([4, 5]) == false`},
		{expr: `[].hasAny([1, 2]) == false`},
		{expr: `[1, 2].hasAny([]) == false`},
		{expr: `[1, 2, 3, 4].hasAll([2, 3]) == true`},
		{expr: `[1, 2, 3].hasAll([2, 4]) == false`},
		{expr: `[1, 2].hasAll([]) == true`},
		{expr: `[].hasAll([1]) == false`},
		{expr: `[].hasAll([]) == true`},
		{expr: `[1, 2].hasOnly([1, 2, 3]) == true`},
		{expr: `[1, 4].hasOnly([1, 2, 3]) == false`},
		{expr: `[].hasOnly([1, 2]) == true`},
		{expr: `[1, 2].hasOnly([]) == false`},
		{expr: `[].hasOnly([]) == true`},
	}
...
func TestListsVersion(t *testing.T) {
	versionCases := []struct {
		...
		},
		{
			version: 4,
			supportedFunctions: map[string]string{
				"hasAny":  "[1, 2, 3].hasAny([2, 4]) == true",
				"hasAll":  "[1, 2, 3, 4].hasAll([2, 3]) == true",
				"hasOnly": "[1, 2].hasOnly([1, 2, 3]) == true",
			},
		},
	}
...
func TestListsCosts(t *testing.T) {
	tests := []struct {
		...
		{
			name:          "list_has_any",
			expr:          `[1, 2, 3].hasAny([2, 4])`,
			estimatedCost: checker.FixedCostEstimate(27),
			actualCost:    27,
		},
		{
			name:          "list_has_any_var",
			expr:          `x.hasAny([1, 2, 3])`,
			vars:          []cel.EnvOption{cel.Variable("x", cel.ListType(cel.IntType))},
		...
		{
			name:          "list_has_only_var",
			expr:          `x.hasOnly([1, 2, 3])`,
			vars:          []cel.EnvOption{cel.Variable("x", cel.ListType(cel.IntType))},
			in:            map[string]any{"x": []int64{1, 2}},
			hints:         map[string]uint64{"x": 10},
			estimatedCost: checker.CostEstimate{Min: 12, Max: 42},
			actualCost:    18,
		},
		{
			name:          "list_has_any_arg_var",
			expr:          `[1, 2, 3].hasAny(y)`,
			vars:          []cel.EnvOption{cel.Variable("y", cel.ListType(cel.IntType))},
			in:            map[string]any{"y": []int64{2, 4}},
			hints:         map[string]uint64{"y": 5},
			estimatedCost: checker.CostEstimate{Min: 12, Max: 27},
			actualCost:    18,
		},
		{
			name: "list_has_any_var_var",
			expr: `x.hasAny(y)`,
			vars: []cel.EnvOption{
				cel.Variable("x", cel.ListType(cel.IntType)),
				cel.Variable("y", cel.ListType(cel.IntType)),
			},
			in:            map[string]any{"x": []int64{1, 2, 3}, "y": []int64{2, 3, 4, 5}},
			hints:         map[string]uint64{"x": 10, "y": 5},
			estimatedCost: checker.CostEstimate{Min: 3, Max: 53},
			actualCost:    15,
		},
		{
			name: "list_has_all_var_var",
			expr: `x.hasAll(y)`,
			vars: []cel.EnvOption{
				cel.Variable("x", cel.ListType(cel.IntType)),
				cel.Variable("y", cel.ListType(cel.IntType)),
			},
			in:            map[string]any{"x": []int64{1, 2, 3, 4, 5}, "y": []int64{2, 3}},
			hints:         map[string]uint64{"x": 10, "y": 5},
			estimatedCost: checker.CostEstimate{Min: 3, Max: 53},
			actualCost:    13,
		},
		{
			name: "list_has_only_var_var",
			expr: `x.hasOnly(y)`,
			vars: []cel.EnvOption{
				cel.Variable("x", cel.ListType(cel.IntType)),
				cel.Variable("y", cel.ListType(cel.IntType)),
			},
			in:            map[string]any{"x": []int64{2, 3}, "y": []int64{1, 2, 3, 4, 5}},
			hints:         map[string]uint64{"x": 10, "y": 5},
			estimatedCost: checker.CostEstimate{Min: 3, Max: 53},
			actualCost:    13,
		},
		{
			name: "list_has_any_list_of_lists",
			expr: `x.hasAny(y)`,
			vars: []cel.EnvOption{
				cel.Variable("x", cel.ListType(cel.ListType(cel.IntType))),
				cel.Variable("y", cel.ListType(cel.ListType(cel.IntType))),
			},
			in: map[string]any{
				"x": []any{[]any{int64(1), int64(2)}, []any{int64(3), int64(4)}, []any{int64(5), int64(6)}},
				"y": []any{[]any{int64(3), int64(4)}, []any{int64(7), int64(8)}},
			},
			hints:         map[string]uint64{"x": 5, "x.@items": 3, "y": 4, "y.@items": 2},
			estimatedCost: checker.CostEstimate{Min: 3, Max: 23},
			actualCost:    9,
		},
		{
			name: "list_has_any_nested_map",
			expr: `x.hasAny(y)`,
			vars: []cel.EnvOption{
				cel.Variable("x", cel.ListType(cel.MapType(cel.StringType, cel.ListType(cel.MapType(cel.StringType, cel.StringType))))),
				cel.Variable("y", cel.ListType(cel.MapType(cel.StringType, cel.ListType(cel.MapType(cel.StringType, cel.StringType))))),
			},
			in: map[string]any{
				"x": []any{map[string]any{"k1": []any{map[string]any{"a": "b"}}}},
				"y": []any{map[string]any{"k1": []any{map[string]any{"a": "b"}}}},
			},
			hints:         map[string]uint64{"x": 6, "y": 4},
			estimatedCost: checker.CostEstimate{Min: 3, Max: 27},
			actualCost:    4,
		},
	}
