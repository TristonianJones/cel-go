...
func TestBindingsMemoryLimitStringConcat(t *testing.T) {
	// A 1MiB input string doubled per bind level: ~104,858 units at 10 bytes per unit, so
	// with a 1M unit (~10MiB) memory limit the trip occurs at the a4 concatenation whose
	// 16MiB output (~1.7M units) exceeds the limit. By then the Go layer has materialized
	// 2+4+8+16 = 30MiB of intermediate strings; the limit halts the remaining doublings,
	// which unchecked would allocate ~510MiB cumulative through a8.
	...
	t.Run("limit_trips_mid_amplification", func(t *testing.T) {
		...
		// The enforcement point follows the materialization of the offending 16MiB value,
		// so ~30MiB of intermediates is the expected floor; the bound proves the remaining
		// exponential trajectory (~510MiB unchecked) was halted.
