...
// Interpretable can accept a given Activation and produce a value.
...
// InterpretableV2 can accept a given ExecutionFrame and produce a value.
...
// exec evaluates an interpretable within the given ExecutionFrame.
func exec(i InterpretableV2, frame *ExecutionFrame) ref.Val {
	return i.Exec(frame)
}
...
func (ne *evalNe) Exec(frame *ExecutionFrame) ref.Val {
	lVal := exec(ne.lhs, frame)
	rVal := exec(ne.rhs, frame)
	if types.IsUnknownOrError(lVal) {
		return lVal
	}
	if types.IsUnknownOrError(rVal) {
...
func (e *evalWatchAttr) Exec(frame *ExecutionFrame) ref.Val {
	val := exec(e.InterpretableAttribute, frame)
...
func (cond *evalExhaustiveConditional) Exec(frame *ExecutionFrame) ref.Val {
	cVal := exec(cond.attr.expr, frame)
