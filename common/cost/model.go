...
func (m minExpr) estimate(ctx EstimateContext) SizeEstimate {
	...
	if contextModelVersion(ctx) < ModelVersion1 {
		// ModelVersion0 reported a floor of 1 for any non-empty interval, ignoring the operands'
...
//
// WithModelVersion is accepted for symmetry with FunctionEstimatorWithOptions but has no effect:
...
func (e *estimatorEvalContext) modelVersion() ModelVersion {
	return e.version
}
