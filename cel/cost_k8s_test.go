...
func testK8sCost(t *testing.T, expr string, expectEstimatedCost cost.CostEstimate, expectRuntimeCost uint64, expectEstimatedCostV0 ...cost.CostEstimate) {
	...
	legacyCost, err := env.EstimateCost(compiled, est, cost.EstimateModelVersion(cost.ModelVersion0))
	if err != nil {
		t.Fatalf("env.EstimateCost(ModelVersion0) failed: %v", err)
	}
	if legacyCost.Min != wantLegacy.Min || legacyCost.Max != wantLegacy.Max {
		t.Errorf("Expected ModelVersion0 estimated cost of %d..%d but got %d..%d", wantLegacy.Min, wantLegacy.Max, legacyCost.Min, legacyCost.Max)
	}
