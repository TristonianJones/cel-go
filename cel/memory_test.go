...
func TestMemoryTracking(t *testing.T) {
	tests := []struct {
		...
		{
			name:  "string_concat_scaled",
			expr:  `s + s`,
			decls: []EnvOption{Variable("s", StringType)},
			in:    map[string]any{"s": strings.Repeat("a", 11)},
			// The 11-char inputs are 2 units each; the peak is the 22-char output at 3 units.
			wantPeak: 3,
		},
		{
...
func TestMemoryTrackingCalculationLimitExceeded(t *testing.T) {
	...
	_, details, err := program.Eval(map[string]any{"a": []int64{1, 2, 3, 4, 5}})
	if err != nil {
		t.Fatalf("program.Eval() failed: %v", err)
	}
